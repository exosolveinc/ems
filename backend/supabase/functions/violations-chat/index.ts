...
serve(async (req) => {
  ...
  try {
    ...
    const since = daysAgo(TIME_RANGES.violations)
    ...
    // Build query - fetch violations without complex joins
    ...
    if (!adminManager) {
      ...
      query = query.limit(500) // Limit for admins
    }
    ...
    // Add employee names to violations for context
    let violationsWithNames = violations || []
    if (adminManager && employees) {
      violationsWithNames = (violations || []).map((v: any) => {
    ...
    const dataForAI = adminManager
      ? { employees, violations: violationsWithNames }
    ...
    const systemPrompt = SYSTEM_PROMPTS.violations(userRole, currentDate)
